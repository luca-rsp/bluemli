...
| sketch-findings-bluemli | Validated design decisions, CSS patterns, and HTML structures from sketch experiments. Auto-load when implementing the gallery surfaces (Phase 2 `/gallery` + `/gallery/<slug>`) or any Phase 3 page (landing pop-up mini-callout, `/popups`, `/about`, `/say-hi`). Covers the editorial-plate detail page, per-status grid colors, the two pop-up render scales, and the no-form `/say-hi` surface. | `.claude/skills/sketch-findings-bluemli/SKILL.md` |
<!-- GSD:skills-end -->
