...
**Domain:** Small static portfolio / marketing site (jewelry studio) on Cloudflare
**Researched:** 2026-05-12
**Confidence:** HIGH (versions and platform paths verified against official docs published Mar–Apr 2026)

## TL;DR

Build with **Astro 6.2** on **Cloudflare Workers + Static Assets** (not Pages — see below), keep all 5 pages prerendered, put the contact form on the same Worker behind a `run_worker_first` route, validate Turnstile server-side, and send the email via **Resend**. Use Astro Content Collections v2 with `glob()` + `file()` loaders for gallery and pop-ups. Skip image-service transformation (Sharp doesn't run in `workerd`) — pre-optimize photos as WebP at commit time with the `<Image />` component and `passthroughImageService()`. Drop the design skill's React JSX components in as **server-rendered Astro islands with no `client:` directive** (zero client JS). Load `colors_and_type.css` once in `BaseLayout.astro`; use Astro's stable Fonts API for Nunito; self-host the headline hand-fonts via the local provider. Install Umami's hosted script with `data-website-id` + `data-domains` in the same BaseLayout.

---

## CRITICAL FINDING: Cloudflare Pages vs Workers

The project's `PROJECT.md` names "Cloudflare Pages" as the host. **In 2026 this is the wrong target for a new Astro project.** Two independent, verified facts:

1. **`@astrojs/cloudflare` v13 dropped Pages support.** The adapter docs explicitly state: *"The Astro Cloudflare adapter no longer supports deployment on Cloudflare Pages."* It targets Cloudflare Workers + Static Assets only.
2. **Cloudflare has deprioritized Pages.** Their announcement of Workers + Static Assets explicitly says: *"Cloudflare Pages will continue to be supported, but, going forward, all of our investment, optimizations, and feature work will be dedicated to improving Workers"* and *"you should start with Workers."*

**Recommendation:** Deploy to **Cloudflare Workers with Static Assets**. Functionally equivalent to Pages for this use case (git integration via the dashboard, free static-asset requests, preview deployments on PRs) and is where the platform is heading. This also collapses the "Pages + separate contact Worker" model into a single Worker that serves the static site and handles `POST /api/contact` — fewer moving parts.

Update `PROJECT.md`'s "Hosting" constraint accordingly during roadmap creation.
...
pnpm add @astrojs/cloudflare@^13.5.0
pnpm add @astrojs/react@^5.0.4
pnpm add react@^19 react-dom@^19
pnpm add resend

# Dev tooling
pnpm add -D wrangler@^4
pnpm add -D prettier@^3 prettier-plugin-astro@^0.14
...
export default defineConfig({
  ...
  fonts: [
    ...
    },
    {
      ...
      src: "/fonts/bluemli-display.woff2",
      weights: [400],
    },
  ],
});
...
```jsonc
{
  ...
  "vars": {
    "RESEND_FROM": "hello@studiobluemli.com",
    "RESEND_TO": "founder@example.com"
  }
  // Secrets (RESEND_API_KEY, TURNSTILE_SECRET) set via `wrangler secret put`
}
```

`run_worker_first: ["/api/*"]` means: requests to `/api/*` invoke the Worker; everything else is served straight from the static-asset bundle without burning Worker requests. This is the modern replacement for Pages Functions.
...
const gallery = defineCollection({
  ...
  schema: ({ image }) =>
    z.object({
      ...
      status: z.enum(["available", "sold", "one-of-one"]),
      description: z.string().max(280),
      photo: image(),                // image() makes it a typed ImageMetadata
      photos: z.array(image()).optional(),
      order: z.number().optional(),  // optional manual sort hint
    }),
});

const popups = defineCollection({
  loader: file("./src/content/popups.yaml"),  // single YAML file, one entry per event
  schema: z.object({
    id: z.string(),
    date: z.coerce.date(),       // string -> Date
    endDate: z.coerce.date().optional(),
    location: z.string(),
    time: z.string(),            // "11am – 5pm"
    description: z.string(),
    url: z.string().url().optional(),
  }),
});

export const collections = { gallery, popups };
```

Past vs upcoming is just a build-time filter on `popups`:
```ts
const all = await getCollection("popups");
const now = new Date();
const upcoming = all.filter(p => p.data.date >= now).sort((a,b) => +a.data.date - +b.data.date);
const past = all.filter(p => p.data.date < now).sort((a,b) => +b.data.date - +a.data.date);
```

## Contact form (target shape)

```ts
// src/pages/api/contact.ts  (server-rendered, not prerendered)
export const prerender = false;

import { Resend } from "resend";

export async function POST({ request, locals }) {
  const env = locals.runtime.env;  // Cloudflare bindings in Astro 6
  ...
  if (!verify.success) return new Response("spam check failed", { status: 400 });

  // 2) Send via Resend
  const resend = new Resend(env.RESEND_API_KEY);
  await resend.emails.send({
    from: env.RESEND_FROM,                    // verified domain on Resend
    to: env.RESEND_TO,                        // founder inbox
    reply_to: data.get("email")?.toString(),
    subject: `Bluemli — message from ${data.get("name")}`,
    text: data.get("message")?.toString() ?? "",
  });

  return Response.redirect(new URL("/say-hi/thanks", request.url), 303);
}
```

Client snippet (in `say-hi.astro`):
```html
...
<script
  ...
  data-domains="studiobluemli.com"
></script>
```

Custom events use plain attributes (no JS needed) — e.g. on the gallery cards:

```html
<a href={`/gallery/${piece.id}`} data-umami-event="gallery-click" data-umami-event-piece={piece.data.name}>
```

Or programmatic: `umami.track('contact-submitted')` after a successful form post on the thanks page.

## Styling approach

The design skill ships `colors_and_type.css`. **Vanilla CSS + Astro scoped styles is right.** No Tailwind, no CSS-in-JS. Reasoning:
...
Pattern in `BaseLayout.astro`:
```astro
---
import "../styles/colors_and_type.css";   // from the design skill — copy in once
import "../styles/site.css";              // your site-level globals (cream bg, base type)
...
<html lang="en">
  <head>
    ...
    <link rel="apple-touch-icon" href="/logo/mark-favicon-180.png">
    <!-- Umami -->
    <slot name="head" />
  </head>
  <body><slot /></body>
...
**If contact-form volume ever exceeds 100 emails/day:**
...
**If the founder asks for a CMS in the future:**
...
**If the founder wants a blog later:**
...
---
*Stack research for: small static portfolio + marketing site (jewelry studio)*
*Researched: 2026-05-12*
