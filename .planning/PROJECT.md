...
- [x] **Live at apex `studiobluemli.com` (and `www.` 301-redirects to apex with path + query preservation) via the existing Cloudflare account** _(Phase 4 — FND-03, LCH-04, LCH-07. Cloudflare Workers + Static Assets serving over HTTPS with valid Google Trust Services cert (expires 2026-08-13); `public/_headers` ships CSP, HSTS-2y+preload directive, Permissions-Policy lockdown, X-CTO, Referrer-Policy, X-Frame-Options frame-ancestors none; HSTS preload-list submission deferred to v1.x per CONTEXT D-09.)_
- [x] **Umami Cloud analytics installed (free tier, single script tag, no consent banner needed)** _(Phase 4 — LCH-01, LCH-02, LCH-03. Env-aware `<script async>` in `BaseLayout.astro` gated on `import.meta.env.PUBLIC_UMAMI_WEBSITE_ID` (build-time, via Workers Builds → Build variables and secrets); `data-domains="studiobluemli.com"` apex-only post-cutover. Six events fire: `gallery_card_click`, `inquire_ig_per_piece` (+ piece slug), `say_hi_ig_dm`, `say_hi_mailto`, `footer_ig_click`, `popups_empty_ig_click`.)_
- [x] **Lighthouse mobile ≥ 90 on Performance/Accessibility/Best Practices/SEO across all 6 routes on production** _(Phase 4 — LCH-05. All 24 cells passed, min 92 / max 100; verified via `scripts/lighthouse-production.sh` against the live apex. LHCI in PR CI hardened from `numberOfRuns: 1` to `numberOfRuns: 3` + `aggregationMethod: median-run` to eliminate single-run noise.)_
- [x] **Open Graph previews render correctly in social unfurls** _(Phase 4 — LCH-06. Pivoted acceptance per RESEARCH Pitfall 4 — Twitter Card Validator deprecated 2022 → Facebook Sharing Debugger + iMessage/IG-DM real-platform unfurl tests; all 5 sub-items verified by founder. `og-default.png` regenerated with full-palette mark per PR #9 after `scripts/generate-og-default.mjs` candidate-order bug caught during LCH-06 live testing.)_
- [x] **"Looks Done But Isn't" launch checklist walked end-to-end** _(Phase 4 — LCH-08. 11 D-04 items ✓ in `LAUNCH-REPORT.md`; founder phone-checks 3/3 YES (IG DM open, mailto open, cellular feel fast); post-cutover housekeeping done — `*.workers.dev` preview entry removed from Umami site list per D-02.)_
...
---
*Last updated: 2026-05-15 — Phase 4 (Analytics, Polish & Launch) complete. Site is LIVE at https://studiobluemli.com. v1.0 milestone complete (4/4 phases). Verifier passed 9/9 must-haves; code review surfaced 0 critical / 5 warning / 7 info findings, all v1.x follow-ups. Mid-cutover doc fixes: PR #7 (SETUP-DNS.md — 3 dashboard-path corrections), PR #8 (footer + popups-empty Umami events, scope grew 4→6), PR #9 (full-palette OG image). Known v1.x follow-ups: REQUIREMENTS.md doc-sync (LCH-03 obsolete "contact-form submit" event reference; LCH-06 Twitter Card Validator → FB Debugger + real-unfurl tests), Cache-Control duplicate-header on static assets (cosmetic, RFC 9111 §5.2-compatible), pre-existing `check-no-hydration.sh` failure (predates Phase 4, logged in deferred-items.md), HSTS preload-list submission deferred to v1.x. Active section no longer tracks Phase 3 items — those should be backfilled to Validated as a separate housekeeping pass. Up next: optional v1.x cleanup phase (gsd-secure-phase to formalize SECURITY.md, follow-ups above) or new milestone.*
